...
static int uxsocket_init()
{
    ...
    if (chmod(IVSHMEM_SOCK_PATH, 0666))
    {
...
static int channel_poll(struct host_proxy *pxy)
{
    ...
    {
        vmid = (pxy->next_vm_poll + 1) % pxy->next_vm_id;
        vm = &pxy->vms[i];
        if (vm != NULL)
        {
            for (j = 0; j < VM_BATCH_SIZE; j++)
            {
        ...
        pxy->next_vm_poll = (vmid + 1) % pxy->next_vm_id;
    }
...
static int app_ctxs_poll(struct host_proxy *pxy)
{
    ...
    n = epoll_wait(pxy->ctx_epfd, evs, 32, 0);
